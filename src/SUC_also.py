...
def solve_SUC(T, num_gen, num_WT, num_branch, load_bus_all, PTDF, gen_cap_individual,
    ...
    if method in ['wcvar', 'wcvar_also']:
    ...
    elif method in ['proposed', 'ori', 'exact', "slfa_also"]:
        # define ancillary variables
        ...
        for p in range(P):
            ...
            if method in ["proposed", "slfa_also", 'exact']:
                N_p = np.where(b_xi_set < q_p)[0]
            elif method == "ori":
                N_p = np.arange(N_WDR)
            else:
                raise ValueError('method can only be either "proposed" or "ori", or "slfa_also".')
        ...
        if len(b_xi_set_all) > 0:
            ...
            if method in ['proposed', 'ori', 'slfa_also']:
        ...
        if method in ['proposed', 'slfa_also']:
        ...
        raise ValueError('method can only be either "proposed" or "ori" or "exact" or "wcvar" or "bonferroni", or "slfa_also", or "wcvar_also".')
    # ---------------------------------------------------
    ...
    if (method in ["wcvar_also", "slfa_also"]) and (not np.isnan(ObjVal)):
        # start the also# algorithm
        # get the upper bound
        t_solve = 0 # only record the ALSO time
        original_obj = prob.ObjVal
        tU = prob.objVal
        # ------------------- step 1 start -----------------------
        # solve quantile relaxation to get the lower bound
        # prob = gp.Model('SUC_lb')
        ...
        #     prob.addConstr(gen_power_all[t, :] == x[len_x_t*t + 2*num_gen:len_x_t*t + 3*num_gen])
        #     prob.addConstr(WT_schedule_all[t, :] == x[len_x_t*t + 3*num_gen:len_x_t*t + 3*num_gen + num_WT])
        # ##  approximate JCC by ICC
        # # the key is to evaluate VaR through bilinear programming, but now we set the same epsilon
        # # epsilon_p_list = epsilon * np.ones(P)
        # # t_solve_var_s = time.time()
        # # # by joblib to speed up the calculation of VaR for Bonferroni approximation
        # # if njobs > 1:
        # #     var_list = Parallel(n_jobs=njobs)(delayed(solve_VaR)(N_WDR, theta, epsilon_p_list[p], random_var_scenarios, B[p]) for p in range(P))
        ...
        # # print(f'spent {t_solve_var} seconds to solve the VaR by bilinear programmes.')
        # # print('---------------')
        # prob.addConstr(A @ x <= d + B @ random_var_scenarios.mean(axis=0)) # EFFICIENTLY approximate the lower bound by using the mean value of the random vector
        ...
        # # get the lower bound
        # tL = prob.ObjVal
        # get the lower bound by 5% reduction of the upper bound
        tL = 0.95 * tU

        # ------------------- step 2 start -----------------------
        ### set the base model so we do not need to build the model again
        prob = gp.Model('loop_1')
        gen_power_all = prob.addMVar((T, num_gen), lb=-GRB.INFINITY, ub=GRB.INFINITY) # the scheduled power output of thermal generators
        gen_UR_all = prob.addMVar((T, num_gen), lb=-GRB.INFINITY, ub=GRB.INFINITY) # the upward reserve of thermal generators
        gen_DR_all = prob.addMVar((T, num_gen), lb=-GRB.INFINITY, ub=GRB.INFINITY) # the downward reserve of thermal generators
        gen_v_all = prob.addMVar((T, num_gen), vtype=GRB.BINARY) # the binary variable indicating the on/off status of thermal generators
        ...
        for t in range(T):
            ...
            prob.addConstr(WT_schedule_all[t, :] == x[len_x_t*t + 3*num_gen:len_x_t*t + 3*num_gen + num_WT])
        ### 40a of ALSO#
        alpha = prob.addMVar(N_WDR, lb=0, ub=GRB.INFINITY)
        beta = prob.addMVar(1, lb= -GRB.INFINITY, ub=0)
        lambda_var = prob.addMVar(1, lb=0, ub=GRB.INFINITY)
        for i in range(N_WDR):
            prob.addConstr(alpha[i] >= (A @ x - B @ random_var_scenarios[i] - d) - beta)
        # add objective
        prob.setObjective(epsilon * beta + lambda_var * theta + gp.quicksum(alpha) / N_WDR, GRB.MINIMIZE)
        prob.addConstr(lambda_var >= norm(B, ord=2, axis=1))
        ### add constraint of the cost
        # the unit commitment cost
        UC = prob.addMVar((T, num_gen), lb=-GRB.INFINITY, ub=GRB.INFINITY)
        # start up cost
        SU = prob.addMVar((T, num_gen), lb=-GRB.INFINITY, ub=GRB.INFINITY)
        ...
        # tU += tol_band
        while True:
            if (tU - tL < tol_band) or (n_iter > max_iter):
                break
            
            t_cur = (tU + tL) / 2

            prob.addConstr(UC.sum() + FC.sum() + RC.sum() + CP.sum() <= t_cur, name='cost_constr')
            ...
            print("########################################")
            print(f"The {n_iter}-th iteration of {method}\n tL = {tL}, tU = {tU}\n current objVal = {t_cur}, original obj = {original_obj}")
            # remove the previous cost constraint
            prob.remove(prob.getConstrByName('cost_constr'))

            # if not optimal, continue
            if prob.status != GRB.OPTIMAL:
                tL = t_cur
                continue
            ###### check the satisfaction of the probabilistic constraint
            # use Chen 2022 OR paper to quantify the worst-case violation probability
            x_value = x.X
            distance_list = []
            for i in range(N_WDR):
                temp = (B @ random_var_scenarios[i] + d - A @ x_value) / norm(B, ord=2, axis=1)
                distance_list.append(max(np.min(temp), 0))
            # sort the distance
            distance_list = np.array(distance_list)
            distance_list_sorted = np.sort(distance_list)
            if distance_list_sorted[0] > theta * N_WDR:
                j_star = 0
            else:
                # j_start is the maximum j such that the sum of the first j distances is smaller than theta*N_WDR
                j_star = np.max(np.where(np.cumsum(distance_list_sorted) <= theta * N_WDR)[0]) + 1
            if j_star == N_WDR:
                prob_satisfy = 0
            else:
                p_star = (theta * N_WDR - np.sum(distance_list_sorted[:j_star])) / distance_list_sorted[j_star]
                prob_satisfy = 1-(j_star + p_star) / N_WDR
            
            if prob_satisfy >= 1 - epsilon - 1e-4:
                tU = t_cur
            else:
                tL = t_cur

            n_iter += 1
            print(f"The WJCC satisfaction probability is {(prob_satisfy)*100}%")
            print("########################################")
    ...
    return prob, x, gen_UR_all, gen_DR_all, gen_v_all,  gen_power_all, WT_schedule_all, WT_curtail_all, A, B, d, t_solve, ObjVal

def main():
    num_gen = 10  # the number of thermal generators
    num_WT = 10  # the number of wind turbines
    T = 2 # the number of time steps
    ...
    method = 'slfa_also' # proposed, ori, exact, wcvar, bonferroni, wcvar_also, slfa_also. the method to reformulate the WDRJCC
    N_WDR = 50 # the number of scenarios for the WDRJCC
    epsilon = 0.4 # the risk level
    theta = 0.1 # the Wasserstein radius. Bonferroni approximation requires small theta for feasibility
    load_scaling_factor = 1.5 # the scaling factor for the load
    ...
    gen_power_all, WT_schedule_all, WT_curtail_all, A, B, d, t_solve, ObjVal = solve_SUC(**input_param_dict)
