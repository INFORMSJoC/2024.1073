...
def WT_sce_gen(WT_no, samples_no, seed = 0):
    ...
    WT_forecast = np.load(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'data', 'GEFC2012_wind_forecast.npy'))
    WT_forecast = WT_forecast.reshape([1, -1, 1])
    WT_forecast = np.tile(WT_forecast, (1, 1, WT_no))

    ## get the WT error
    WT_error_samples = np.load(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'data', 'GEFC2012_temporal_correlated_error_samples.npy'))
