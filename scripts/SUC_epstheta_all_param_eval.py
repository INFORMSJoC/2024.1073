...
import numpy as np
import pandas as pd
from SUC_epstheta import solve_SUC
...
def solve_one_instance(param, save_path_root, quadra_cost, bigM, thread):
    ...
    # ------------------
    # parameters
    network_name, load_scaling_factor, epsilon, theta, T, num_gen, N_WDR, gurobi_seed, method, bearable_cost_prop = param
    # ------------------
    ...
    log_file_name = (f'{network_name}_theta{theta}_epsilon{epsilon}_gurobi_seed{gurobi_seed}'
                     f'_num_gen{num_gen}_N_WDR{N_WDR}_load_scaling_factor{load_scaling_factor}_{method}_T{T}{"quadra_cost" if quadra_cost else ""}_bc{bearable_cost_prop}.txt')
    ...
    result_dict_path = (f'result_{network_name}_theta{theta}_epsilon{epsilon}_gurobi_seed{gurobi_seed}'
                        f'_num_gen{num_gen}_N_WDR{N_WDR}_load_scaling_factor{load_scaling_factor}_{method}_T{T}{"quadra_cost" if quadra_cost else ""}_bc{bearable_cost_prop}.npy')
    ...
    input_param_dict = {'T': T, 'num_gen': num_gen, 'num_WT': num_WT, 'num_branch': num_branch,
                        ...
                        'urc': urc, 'drc': drc, 'su': su, 'sd': sd, 'bigM': bigM, 'gurobi_seed': gurobi_seed, 'log_file_name': log_file_name, 'rng': rng, 'quadra_cost': quadra_cost, 'bearable_cost_prop': bearable_cost_prop}
    ...
    if np.isnan(min_cost):
        ...
        reliability_test_base = 100*np.mean(np.all(A @ x_base <= random_var_scenarios_test @ B.T + d, axis=1))
        print('------------------------------------')
    ...
    # save the results
    result_dict = {'min_cost (USD)': min_cost, 'reliability_test (%)': reliability_test, 't_solve (s)': t_solve, "min_cost_base (USD)": min_cost_base, "object_value": prob.objVal if prob.status == GRB.OPTIMAL else np.nan, 'reliability_test_base (%)': reliability_test_base,}
...
def run_all_param():
    ...
    n_jobs = 30 # higher than 20 would cause memory issues
    # ---------------------------------------------------------------------------------
    T_list = [1, 2, 3, 4, 5]  # the number of time steps
    theta_list = [1e-1]  # the Wasserstein radius. Bonferroni approximation requires small theta for feasibility
    epsilon_list = [0.2, 0.1]  
    ...
    method_list = ['proposed_eps', 'ori_eps']  
    network_name_list = ['case24_ieee_rts']
    quadra_cost = True

    # find the combination of all these parameters
    param_comb = list(itertools.product(network_name_list, load_scaling_factor_list, epsilon_list, theta_list, T_list, num_gen_list, N_WDR_list, gurobi_seed_list, method_list, bearable_cost_prop_list))

    ### having another set of combinations for comparing theta-based method
    T_list = [6, 8, 10, 12, 14]
    bearable_cost_prop_list = [1.01, 1.03, 1.05]  #
    epsilon_list = [0.1]
    theta_list = [1e-1]
    method_list = ['proposed_eps', 'proposed_theta']
    param_comb += list(itertools.product(network_name_list, load_scaling_factor_list, epsilon_list, theta_list, T_list, num_gen_list, N_WDR_list, gurobi_seed_list, method_list, bearable_cost_prop_list))

    save_path_root = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'results', f'SUC_epstheta_results_bigM{int(bigM)}_thread{int(thread)}')
