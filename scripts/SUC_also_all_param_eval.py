...
import numpy as np
import pandas as pd
from SUC_also import solve_SUC
...
def solve_one_instance(param, save_path_root, quadra_cost, bigM, thread):
    ...
    prob, x, gen_UR_all, gen_DR_all, gen_v_all, \
        gen_power_all, WT_schedule_all, WT_curtail_all, A, B, d, t_solve, min_cost = solve_SUC(**input_param_dict)
...
def run_all_param():
    ...
    method_list = ['proposed', 'exact', 'ori', 'wcvar', 'bonferroni', 'slfa_also']  # proposed, ori, exact, wcvar, bonferroni, slfa_also, wcvar_also. the method to reformulate the WDRJCC
    ...
    save_path_root = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'results', f'SUC_ALSO_results_bigM{int(bigM)}_thread{int(thread)}')
